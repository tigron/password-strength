...
class Hive2024 extends \Password\Ruleset {
	...
	public static function create(): self {
		...
		// Numbers only
		$rule = new \Password\Rule\Regex(\Password\Strength::VULNERABLE);
		$rule->regexp = '^[\d\h]{1,6}\z$';
		$ruleset->add_rule($rule);

		$rule = new \Password\Rule\Regex(\Password\Strength::INSECURE);
		$rule->regexp = '^[\d\h]{7,14}\z$';
		$ruleset->add_rule($rule);

		$rule = new \Password\Rule\Regex(\Password\Strength::ACCEPTABLE);
		...
		$rule = new \Password\Rule\Regex(\Password\Strength::GOOD);
		$rule->regexp = '^[a-z\h]{14,16}\z$';
		$ruleset->add_rule($rule);

		$rule = new \Password\Rule\Regex(\Password\Strength::STRONG);
